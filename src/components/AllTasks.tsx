...
export default function AllTasks({ date }: AllTasksProps) {
    ...
    const { tasks, fetchTasksLoading, error } = useSelector((state: RootState) => state.tasks);
    ...
    const filteredTasks = filteredTasksByDate.filter((task) => {
        ...
        if (tabValue === 1) return !task.is_completed; // Open tasks
        if (tabValue === 2) return task.is_completed; // Closed tasks
        return true;
    });
    ...
    const openTasksCount = filteredTasksByDate.filter((task) => !task.is_completed).length;
    const closedTasksCount = filteredTasksByDate.filter((task) => task.is_completed).length;
    ...
    return (
        ...
        >
            ...
            {reversedTasks.length === 0 && (
                <Typography variant="body1" textAlign="center" sx={{ mt: 2 }} color='error'>
                    No task
                </Typography>
            )}
        </Box>
